...
const FavePreview = () => {
    ...
    const filteredRecipes = recipes.filter((recipe) => {
      ...
      const matchesBudgetRange =
        filters.budgetRange === 'all' || recipe.budgetRange === filters.budgetRange;
      const hasAllergies = !recipe.allergies || allergies.every((allergy) => !recipe.allergies.includes(allergy));

      return matchesFoodType && matchesMealTime && matchesIngredient && matchesBudgetRange && hasAllergies && recipe.favorited;
    });

  const handleReadRecipe= (recipeId) => {
    sessionStorage.setItem('recipeId', recipeId);
    sessionStorage.setItem('prev', 'favorites');
